import 'package:emart_app/consts/consts.dart';

const socialIconList = [icFacebookLogo, icGoogleLogo, icTwitterLogo];

const sliderList = [imgSlider1, imgSlider2, imgSlider3, imgSlider4];

const secondSliderList = [imgSs1, imgSs2, imgSs3, imgSs4];

const featuredIcon1 = [imgS1, imgS2, imgS3];
const featuredIcon2 = [imgS3, imgS4, imgS5];

const featuredTitle1 = [womenDress, girlsDress, womenGlass];
const featuredTitle2 = [tShirts, boyssWatches, mobilePhones];
const categoryImageList = [
  imgFc1,
  imgFc2,
  imgFc3,
  imgFc4,
  imgFc5,
  imgFc6,
  imgFc7,
  imgFc8,
  imgFc9
];
const categoryTitleList = [
  womenClothing,
  menClothingFashion,
  compAccess,
  autoMmobile,
  kidToy,
  sport,
  jewellery,
  cellPhone,
  furniture
];
